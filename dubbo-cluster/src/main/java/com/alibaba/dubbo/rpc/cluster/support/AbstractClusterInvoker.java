...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        Invoker<T> invoker = doSelect(loadbalance, invocation, invokers, selected); // 在loadbalance中，将上一步的能正常执行的invokers根据配置的负载均衡策略算法，挑选出需要执行的invoker
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        if (invokers == null || invokers.isEmpty())
            return null;
        if (invokers.size() == 1)
            return invokers.get(0);// 只有一个invokers，直接返回，不用loadbalance进行选择
        ...
        Invoker<T> invoker = loadbalance.select(invokers, getUrl(), invocation);// 多个备选的invokers，即有两个服务提供者，则使用loadbalance进行选择（涉及到负载均衡算法）,挑选出需要执行的invoker
